...
生产环境可把 `dist/` 放到静态站点根目录，也可运行 `node server.mjs`（Node 18+），由 `HOST`、`PORT` 设置地址和端口。部署服务器提供 `dist/` 静态文件、`/api/catalog/` 只读目录查询和 `/api/horizons` JPL Horizons 观测代理（目标、UTC 时间、经纬度和海拔作为参数，结果短时缓存）。Horizons 代理不可用时，浏览器继续使用本地 Astronomy Engine 结果。全量搜索另需系统 Python 3（含 SQLite FTS5/trigram）和 `data/catalog.sqlite`。仅使用静态托管时，抽样点云和下载仍可用，全量搜索/目录对象跟随与 Horizons 查询需要接入本项目 API。目录中的 `.csv.gz` 必须作为 gzip 文件发送，不能设置 `Content-Encoding: gzip` 将其误当作预压缩 CSV；本项目开发、预览及生产服务已处理这一点。
...
- 地面望远镜观测：选择北京、上海、莫纳克亚山或自定义经纬度，计算太阳系目标的地平坐标（方位角/高度角）、赤经/赤纬、距离、光行时、太阳高度和当前可观测性；可选调用 JPL Horizons 返回精密观测星历，目录小天体暂不参与本地地平坐标解算。
...
左侧天体目录中的缩略图会直接复用对应二维贴图；只有 GLB 模型的火卫一、火卫二、阋神星、妊神星、鸟神星和灶神星使用从本地模型渲染生成的 128×128 缩略图，图标与场景模型保持一致。
