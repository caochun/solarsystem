...
基于 OpenSpace 资源的浏览器太阳系展示与交互应用，独立于 OpenSpace C++ 桌面程序。当前可选择 **46 个天体**：太阳、八大行星、27 颗天然卫星、五颗矮行星和另外五个小天体；另附 OpenSpace 引用的全部 16 类 JPL 小天体目录。
...
数值、数据与 API 测试共 21 项，覆盖 CSPICE 参考状态、逆行/高偏心率轨道、46 个天体父子关系与比例、尺寸修正、所有完整目录解压校验，以及漫游路径在三个日期与横竖屏下的连续性、有限值和天体/行星环避障。原生验证另对 26 个 SPK 段各取三个时间检查读取结果；这验证读取与换算，不代表长期二体轨道准确。
