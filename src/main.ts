...
async function renderObservatoryImage(
    ...
    const centerAzimuth = pointing?.azimuthDeg ?? result.azimuthDeg;
    ...
    for (const star of stars) {
        ...
        const y = canvas.height / 2 - ((star.altitudeDeg - centerAltitude) / fieldDeg) * canvas.height;
    ...
    if (result.aboveHorizon) {
        ...
        let targetDeltaAz = result.azimuthDeg - centerAzimuth;
        while (targetDeltaAz > 180) targetDeltaAz -= 360;
        while (targetDeltaAz < -180) targetDeltaAz += 360;
        const targetX = canvas.width / 2 + (targetDeltaAz / fieldDeg) * canvas.width + (Math.random() - 0.5) * jitterPixels;
        const targetY = canvas.height / 2 - ((result.altitudeDeg - centerAltitude) / fieldDeg) * canvas.height + (Math.random() - 0.5) * jitterPixels;
        const seeingPixels = (seeingArcsec / 3600 / fieldDeg) * canvas.width;
...
async function queryHorizons() {
    const target = q<HTMLSelectElement>("#observatory-target-select").value as BodyId;
    const codes: Partial<Record<BodyId, string>> = { sun: "10", mercury: "199", venus: "299", moon: "301", earth: "399", mars: "499", jupiter: "599", saturn: "699", uranus: "799", neptune: "899" };
    const code = codes[target];
    const precision = q("#observatory-precision");
    if (!code) { precision.textContent = "当前目标没有可用的 Horizons 目标编号。"; return; }
    const site = { latitude: Number(q<HTMLInputElement>("#observatory-lat").value), longitude: Number(q<HTMLInputElement>("#observatory-lon").value), elevation: Number(q<HTMLInputElement>("#observatory-height").value) };
    const start = new Date(time - 60_000).toISOString();
    const stop = new Date(time + 60_000).toISOString();
    const params = new URLSearchParams({ target: code, start, stop, lat: String(site.latitude), lon: String(site.longitude), elevation: String(site.elevation) });
    precision.textContent = "正在查询 JPL Horizons…";
    try {
...
q<HTMLButtonElement>("#open-observatory").addEventListener("click", () => {
    observatoryPointingAnchor = null;
...
q<HTMLButtonElement>("#observatory-horizons").addEventListener("click", () => void queryHorizons());
...
q<HTMLSelectElement>("#observatory-target-select").addEventListener("change", () => {
    observatoryPointingAnchor = null;
    solveObservatory();
});
...
q<HTMLSelectElement>("#observatory-pointing-mode").addEventListener("change", () => {
    observatoryPointingAnchor = null;
});
q<HTMLButtonElement>("#observatory-lock-pointing").addEventListener("click", () => {
    const site: ObservatorySite = {
        ...observatorySite,
        latitude: Number(q<HTMLInputElement>("#observatory-lat").value),
        longitude: Number(q<HTMLInputElement>("#observatory-lon").value),
    ...
    q<HTMLSelectElement>("#observatory-pointing-mode").value = "fixed";
    solveObservatory();
});
