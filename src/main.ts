...
function renderObservatoryImage(result: ReturnType<typeof observe>, site: ObservatorySite) {
    ...
    note.textContent = `模拟图像 · ${fieldDeg.toFixed(2)}° 视场 · ${exposure.toFixed(2)} s · 增益 ${gain.toFixed(1)} · 读出噪声 ${readNoise.toFixed(1)} ADU · 视宁度 ${seeingArcsec.toFixed(1)}″ · 抖动 ${jitterArcsec.toFixed(1)}″`;
    note.hidden = false;
}
