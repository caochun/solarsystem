...
function renderObservatoryImage(result: ReturnType<typeof observe>, site: ObservatorySite) {
    ...
    for (const star of stars) {
        ...
        const intensity = Math.max(0.08, Math.min(1, (1.3 - star.magnitude * 0.13) * Math.sqrt(exposure * Math.max(0.2, gain))));
        const radius = Math.max(0.5, Math.min(2.8, 3.2 - star.magnitude * 0.45));
    ...
    if (result.aboveHorizon) {
        const targetRadius = Math.max(3, Math.min(105, (result.angularDiameterArcsec / 3600 / fieldDeg) * canvas.width * 0.5));
        const targetIntensity = Math.max(0.35, Math.min(1, (1.4 - result.visualMagnitude * 0.04) * Math.sqrt(exposure * Math.max(0.2, gain))));
        const glow = context.createRadialGradient(canvas.width / 2, canvas.height / 2, 0, canvas.width / 2, canvas.height / 2, targetRadius * 2.5);
        glow.addColorStop(0, `rgba(255,224,160,${targetIntensity})`);
        glow.addColorStop(1, "rgba(255,224,160,0)");
        context.fillStyle = glow;
        context.fillRect(canvas.width / 2 - targetRadius * 2.5, canvas.height / 2 - targetRadius * 2.5, targetRadius * 5, targetRadius * 5);
        context.globalAlpha = targetIntensity;
        context.fillStyle = "#fff0c2";
        context.beginPath();
        context.arc(canvas.width / 2, canvas.height / 2, targetRadius, 0, Math.PI * 2);
        context.fill();
        context.globalAlpha = 1;
    }
    ...
    note.textContent = `模拟图像 · ${fieldDeg.toFixed(2)}° 视场 · ${exposure.toFixed(2)} s · 增益 ${gain.toFixed(1)} · 读出噪声 ${readNoise.toFixed(1)} ADU`;
    note.hidden = false;
}
