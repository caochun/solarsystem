...
KERNELS = {
    ...
    "saturn": (["http://liu-se.cdn.openspaceproject.com/files/solarsystem/planets/saturn/kernels/2/sat441.bsp", "http://liu-se.cdn.openspaceproject.com/files/solarsystem/planets/saturn/kernels/3/sat454.bsp", "http://liu-se.cdn.openspaceproject.com/files/solarsystem/planets/saturn/kernels/3/sat455.bsp"], 699, 37931207.8),
    ...
    for parent, (urls, parent_naif, mu) in KERNELS.items():
        ...
        for identifier, display, asset in names(FOLDERS[parent]):
            ...
            try:
                ...
                orbit = elements(state, mu, epoch_et, epoch_ms)
            except Exception as error:
                print(f"skip {parent}/{identifier}: {error}"); continue
            output.append({"id": identifier.lower(), "name": display, "english": identifier.upper(), "parent": parent, "orbit": orbit, "stateKm": [float(x) for x in state], "source": kernel.url, "sourceEpoch": datetime.fromtimestamp(epoch_ms / 1000, timezone.utc).isoformat().replace("+00:00", "Z")})
